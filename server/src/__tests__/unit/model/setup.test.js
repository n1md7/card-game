...
describe('Test setup CRUD operations', () => {
  ...
  it('should fail with the exception message', () => {
    // which does not exist
    const tmpUserId = 'U-0000';
    expect(() => setup.leaveRoom(tmpUserId)).toThrowError(`could not find a user with the id:${tmpUserId}`);
  });
  ...
  it('should fail with the exception message', () => {
    ...
    // which does not exist
    const tmpUserId = 'U-0000';
    expect(() => setup.leaveRoom(tmpUserId)).toThrowError(/^could not find a user with the id/);
  });
});
